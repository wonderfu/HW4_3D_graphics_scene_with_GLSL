...
void main (void)
{
    ...
    intensity = 0.7 * abs(2.0 * intensity - 0.35);
    intensity = clamp(intensity, 0.0, 1.0);
	
    alpha = fract((Position.y+Extent)*0.65);
	
    color = mix(FireColor1, FireColor2, intensity*0.7) * (1.0 - alpha) * 2.0;
    color = clamp(color, 0.0, 1.0);
    alpha = 1.0 - alpha  * intensity;
    alpha *= alpha;

	//intensity = clamp(noisevec[0], 0.0, 1.0);
	//color = mix(FireColor1, FireColor2, intensity);
